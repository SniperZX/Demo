...
class ShimmerLayoutKt @JvmOverloads constructor(
    context: Context,
    ...
    private var shimmerAnimationDuration = 0L
    ...
    fun setShimmerAnimationDuration(durationMillis: Int) {
        shimmerAnimationDuration = durationMillis.toLong()
        resetIfStarted()
    }
    ...
    private fun dispatchDrawShimmer(canvas: Canvas) {
        ...
        if (canvasForShimmerMask == null && localMaskBitmap != null) {
    ...
    private fun createShimmerPaint() {
        ...
        val maskBitmapShader =
            localMaskBitmap?.let { BitmapShader(it, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP) }
        val composeShader =
            maskBitmapShader?.let { ComposeShader(gradient, it, PorterDuff.Mode.DST_IN) }
    ...
    private fun getShimmerAnimation(): Animator?{
        ...
        if (getWidth() > maskRect?.width()?:0) {
            animationFromX = -animationToX;
        } else {
            animationFromX = -(maskRect?.width()?:0);
        }
        val shimmerBitmapWidth = maskRect?.width()?:0
        val shimmerAnimationFullLength = animationToX - animationFromX

        if (isAnimationReversed){
            maskAnimator = ValueAnimator.ofInt(shimmerAnimationFullLength, 0)
        }else{
            maskAnimator = ValueAnimator.ofInt(0, shimmerAnimationFullLength)
        }
        maskAnimator?.setDuration(shimmerAnimationDuration)
        maskAnimator?.setRepeatCount(repeatCount)
    ...
    companion object {
        ...
        private const val DEFAULT_REPEATCOUNT = ObjectAnimator.INFINITE
    ...
    init {
        ...
        try {
            ...
            repeatCount = a.getInt(R.styleable.ShimmerLayout_shimmer_repeat_count, -1)
        } finally {
            a.recycle()
        }
